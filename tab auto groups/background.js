...
function splitSearchContent(title){
  //let showContent = title.split('"')[1].split(' ')[0];

  // 分割
  let showContent = title.split('q=')[1].split('&')[0];
  showContent = showContent.replaceAll("+",replacePlus);
  if (showContent.length > GroupTitleMaxLength){
    showContent = showContent.substr(0,GroupTitleMaxLength);
  }
  return (showContent);
}
  ...
  (tabId, changeInfo, tab) {
    ...
    if (tab.groupId==-1){
      if (ifstrInList(domain,googlesearchDomain)){
        ...
        chrome.tabs.group(groupOption,(groupId) => {
          //改变分组名字
          chrome.tabGroups.update(
            groupId,
            {
              title :splitSearchContent(tab.title),
            },
          )
        });
      }
