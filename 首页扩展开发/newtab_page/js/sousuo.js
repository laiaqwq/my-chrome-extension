! function() {
    ...
    function zi() {
        ...
        var a = document.querySelector('input[name="type"][="' + bq() + '"]');
        a && (a.checked = !0, l(a))
    }    		
    	}
    function z() {
        return document.querySelector('input[name="type"]:checked').getAttribute("")
    }
    function h() {
        d.checked = s()
    }
    ...
    function r(a) {
        return a.preventDefault(), "" == c.value ? (c.focus(), !1) : (w(t() + c.value), x(s()), s() ? window.open(b.action,target="_self", +new Date) : location.href = b.action, void 0)
        //改为当前页跳转
    }
